...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/v2.4.4.tar.gz https://github.com/holohavij/3x-ui/archive/refs/tags/v2.4.4.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/holohavij/3x-ui/archive/refs/tags/v2.4.4.tar.gz"
        echo -e "Beginning to install x-ui $1"
        wget -N --no-check-certificate -O /usr/local/v2.4.4.tar.gz ${url}
        if [[ $? -ne 0 ]]; then
